...
class StarGAN_multi_step(nn.Module):
    ...
    def forward(self, image_and_cover):
        ...
        for step_id in range(1,4):
            k = torch.randint(0, 5, (1,), device=self.device).item()
            modified_label[:,k] = 1 - modified_label[:,k]
            current_image = self.generator(current_image, modified_label)
            results.append((current_image, step_id))
