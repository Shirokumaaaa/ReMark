...
class Network:
    ...
    def train_denoise1(self, 
        ...
        # 加载权重
        state_dict = torch.load('/home/ldy/..workspace/zhou/repair/autoencoder_epoch_160.pth', map_location=self.device)
        ...
        # ---------------------载入模型并完成fake image集的构建-----------------------------------------------------
        data_pair = []
        acc_watermark_inter = [0.0] * 32  # Increase size to handle max possible step_id
        for _, (images, labels) in tqdm(enumerate(train_dataloader), total=len(train_dataloader), desc="data preparing"):
            images = images.to(self.device)
            message = torch.Tensor(np.random.choice([0,1], (images.shape[0], message_length))).to(self.device)
            with torch.no_grad():
                encoded_image = self.encoder_decoder.module.encode_to_image(images, message)  # 使用 .module 访问方法
            stargan_results = stargan_layer((encoded_image, images, labels))
            ...
            if transformed2_images:
                transformed2_images = torch.cat(transformed2_images, dim=0)
                transformed2_messages = torch.cat(transformed2_messages, dim=0)
                image_total.append(transformed2_images)
                messages_list.append(transformed2_messages)
                damage_list.append(2)  # transformed2阶段的损坏程度为2
            if transformed3_images:
                transformed3_images = torch.cat(transformed3_images, dim=0)
                transformed3_messages = torch.cat(transformed3_messages, dim=0)
                image_total.append(transformed3_images)
                messages_list.append(transformed3_messages)
                damage_list.append(3)  # transformed3阶段的损坏程度为3
            # 合并所有图像和消息
            ...
            for i, damage_level in enumerate(damage_list[1:]):  # 跳过clean，从damage_list[1]开始
                ...
                clean_damage_pairs.append((clean_latent, damage_latent, damage_level,message))
                with torch.no_grad():
                    ima = autoencoder.module.decoder(damage_latent)
                    msg = self.encoder_decoder.module.decode_from_image(ima) if n_gpus > 1 else self.encoder_decoder.decode_from_image(ima)
                    acc = 1.0 - self.decoded_message_error_rate_batch(mess, msg)
                    print(f"Damage level: {damage_level}, Accuracy: {acc:.4f}")
            ...
            for clean_latent, damage_latent, damage_level,message in clean_damage_pairs:
                ...
                for idx, t in enumerate(torch.linspace(0, 1, total_steps+2)):
                    interp_latent = self.slerp(t, clean_latent, damage_latent)
                    step_id = idx + 1
                    inter_img = autoencoder.module.decoder(interp_latent) if n_gpus > 1 else autoencoder.decoder(interp_latent)
                    
                    pred_message = self.encoder_decoder.module.decode_from_image(inter_img) if n_gpus > 1 else self.encoder_decoder.decode_from_image(inter_img)
                    acc_t1 = 1.0 - self.decoded_message_error_rate_batch(message, pred_message)
                    if step_id < len(acc_watermark_inter):
                        acc_watermark_inter[step_id] = acc_t1
                    interpolated_latents.append(interp_latent)
                    interpolated_steps.append((damage_level, step_id, float(t)))
                ...
                for j in range(len(interpolated_latents) - 1):
                    ...
                    current_step = interpolated_steps[j][2]
                    next_step = interpolated_steps[j + 1][2]
                    for idx, (current_img, next_img) in enumerate(zip(current_latent, next_latent)):
                        # 在这个位置把batch消掉
                        # 组合 (当前步latent, 下一步latent, 当前步t, 下一步t, damage_level)
                        # 确保每个样本对应正确的clean_image和message
                        single_clean_image = clean_images[idx:idx+1] if idx < clean_images.shape[0] else clean_images[0:1]
                        single_message = message[idx:idx+1] if idx < message.shape[0] else message[0:1]
                        
                        # print(f"Pairing: {current_img.shape}, {next_img.shape}, {current_step}, {next_step}, {damage_level}")
                        data_pair.append((current_img, next_img, current_step, next_step, damage_level, single_clean_image.squeeze(0), single_message.squeeze(0)))
        print(f"Data pair length: {len(data_pair)}")
        for i in range(len(acc_watermark_inter)):
            print(f"Interpolation step {i+1} watermark accuracy: {acc_watermark_inter[i]:.4f}")
        # 将data_pair转换为Tensor并创建DataLoader
        repair_latents_tensor = torch.stack([pair[0] for pair in data_pair])
        repair_latents_prev_tensor = torch.stack([pair[1] for pair in data_pair])
        repair_t_tensor = torch.tensor([pair[2] for pair in data_pair], dtype=torch.float32)
        ...
        repair_dataloader = DataLoader(repair_dataset, batch_size=32, shuffle=True,drop_last=True)
        # 切分训练集和验证集
        train_size = int(0.8 * len(repair_dataset))
        val_size = len(repair_dataset) - train_size
        train_dataset, val_dataset = torch.utils.data.random_split(repair_dataset, [train_size, val_size])
        ...
        for epoch in range(num_epochs):
            model.train()
            epoch_loss = 0.0
            for step, (repair_latents, repair_latents_prev, repair_t, repair_t_next, repair_damage, clean_image, message) in tqdm(enumerate(train_dataloader), desc=f"Epochs: {epoch+1}/{num_epochs} Train", total=len(train_dataloader)):
                # 将数据移动到设备
                repair_latents = repair_latents.to(self.device)
                repair_latents_prev = repair_latents_prev.to(self.device)
                repair_t = repair_t.to(self.device)
            ...
            if more_val:
                ...
                with torch.no_grad():
                    for val_step, (val_repair_latents, val_repair_latents_prev, val_repair_t, val_repair_t_next, val_repair_damage, val_clean_images, val_messages) in tqdm(enumerate(val_dataloader), desc=f"Epochs: {epoch+1}/{num_epochs} Val", total=len(val_dataloader)):
                        ...
                        for i in range(1,10):
                            ...
                            val_clean_img_np = val_clean_images.detach().cpu().numpy()
                            ...
                            acc = 1.0 - self.decoded_message_error_rate_batch(val_messages, pred_message)
                            print(acc)
                            acc_watermark[i] += acc
                ...
                acc_watermark = 0.0
                with torch.no_grad():
                    for val_step, (val_repair_latents, val_repair_latents_prev, val_repair_t, val_repair_t_next, val_repair_damage, val_step) in tqdm(enumerate(val_dataloader), desc=f"Epochs: {epoch+1}/{num_epochs} Val", total=len(val_dataloader)):
                        # 将数据移动到设备
                        val_repair_latents = val_repair_latents.to(self.device)
                        val_repair_latents_prev = val_repair_latents_prev.to(self.device)
                        val_repair_t = val_repair_t.to(self.device)
...
from zhou.repair.network.Network_old import *
