...
def infer(args):
    ...
    for src_im_file in src_img_list:
        ...
        for drive_im_file in drive_img_list:
            ...
            drive_im_orig_pil = Image.open(drive_img_path).convert("RGB")
            drive_im_pil = drive_im_orig_pil
            drive_im_orig_np = np.array(drive_im_orig_pil)
            orig_h, orig_w = drive_im_orig_np.shape[:2]
            ...
            image = pipe(
                ...
                num_inference_steps=args.num_inference_steps, generator=generator, guidance_scale=args.guidance_scale
            ).images[0]
            ...
            if args.compose_to_original:
                swap_np = (swap_res_tensor[0] * 127.5 + 128).detach().cpu().numpy().astype('uint8').transpose(1, 2, 0)
                blend_np = blend_mask[0, 0].detach().cpu().numpy().astype(np.float32)
                inv_warp = cv2.invertAffineTransform(warp_mat_crop.astype(np.float32))
                swap_back = cv2.warpAffine(
                    swap_np,
                    inv_warp,
                    (orig_w, orig_h),
                ...
                blend_back = np.clip(blend_back, 0.0, 1.0)[..., None]
                comp_np = (
                    swap_back.astype(np.float32) * blend_back
                    + drive_im_orig_np.astype(np.float32) * (1.0 - blend_back)
                )
                images_res_pil = Image.fromarray(np.clip(comp_np, 0.0, 255.0).astype('uint8'))
            else:
                images_res_pil = Image.fromarray((swap_res_tensor[0]*127.5+128).cpu().numpy().astype('uint8').transpose(1,2,0))
            images_res_pil.save(os.path.join(save_swap_path, f'{src_im_name}_{drive_im_name}.png'), quality=100)
...
if __name__ == '__main__': 
    ...
    parser.add_argument('--compose_to_original', dest='compose_to_original', action='store_true')
    parser.add_argument('--no_compose_to_original', dest='compose_to_original', action='store_false')
    parser.set_defaults(compose_to_original=True)
