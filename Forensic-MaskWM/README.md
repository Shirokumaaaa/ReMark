...
This repo can also train on plain image folders (without COCO annotations).

1. Use the provided config:
   ```bash
   configs/train/train_celebahq.yaml
   ```
2. Ensure your data layout is:
   ```text
   Dataset-CelebA_HQ/
   ├── train/
   ├── val/
...
3. Start training:
   ...
   python3 train.py \
       --model_name "D_32bits" \
       --train_config_path "configs/train/train_celebahq.yaml" \
       --model_config_path "configs/model/D_32bits.yaml"
   ```

Note:
- For image-folder training, set `segm_proba: 0.0` (already set in `train_celebahq.yaml`) since no segmentation annotations are used.
