...
class CocoImageIDWrapper(CocoDetection):
    def __init__(self, root, annFile, transform=None, mask_transform=None, random_nb_object=True, max_nb_masks=3, multi_w=False, is_train=True):
        if maskUtils is None:
            raise ImportError("pycocotools is required for COCO-format MaskWM training.")
...
def custom_collate(batch: list) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    if max_masks == 1:
        ...
        if has_paths:
            return images, masks, list(paths)
        return images, masks
...
class ImageFolderMaskWrapper(Dataset):
    """
    Generic image-folder dataset for non-COCO training.
    Returns (image, dummy_mask) to keep the same training loop contract.
    """

    def __init__(self, root, transform=None, image_size=256):
        self.root = root
        self.paths = get_image_paths(root)
    ...
    def __getitem__(self, index):
        ...
        # Dummy mask is only used when segm_proba > 0.
        # For image-folder training set segm_proba=0 to avoid segmentation branch.
        mask = torch.zeros((1, self.image_size, self.image_size), dtype=torch.float32)
        return img, mask, path


def get_dataloader_images(
    data_dir: str,
    image_size: int,
    batch_size: int = 128,
    shuffle: bool = True,
    ...
    """Get dataloader for plain image-folder datasets (e.g., CelebA-HQ)."""
    transform = Compose([
        Resize(image_size),
        CenterCrop(image_size),
        ToTensor(),
    ...
    dataset = ImageFolderMaskWrapper(
        root=data_dir,
        transform=transform,
