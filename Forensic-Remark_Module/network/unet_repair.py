...
# ── LDM 工具函数本地实现（避免复杂的路径依赖）────────────────────────────────────

def checkpoint(func, args, flag):
    """梯度检查点：用于节省显存。"""
    if flag:
        return th.utils.checkpoint.checkpoint(func, *args, use_reentrant=False)
    else:
        return func(*args)
...
def timestep_embedding(timesteps, dim, max_period=10000):
    ...
    :param timesteps: 一维张量，形状为 (batch_size,)
    :param dim: 嵌入维度
    :param max_period: 最大周期
    :return: 形状为 (batch_size, dim) 的嵌入
    """
    half = dim // 2
    freqs = th.exp(
...
class SpatialTransformer(nn.Module):
    ...
    空间变换器（简化版占位符）。
    """
    def __init__(self, in_channels, n_heads=8, d_head=64, context_dim=None, **kwargs):
        super().__init__()
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None,**kwargs):
        ...
        # print(f"emb shape: {emb.shape}, x shape: {x.shape}, context shape: {context.shape if context is not None else None}")
        h = x.type(self.dtype)
...
# ── build_unet 函数（兼容 train_stage2.py）────────────────────────────────────

def build_unet(cfg, latent_channels: int):
    """
    从 config 构建 U-Net。
    
    为了兼容 train_stage2.py，这里导入并使用 unet.py 中的 LatentUNet。
    """
    from network.unet import LatentUNet, _as_int_list
