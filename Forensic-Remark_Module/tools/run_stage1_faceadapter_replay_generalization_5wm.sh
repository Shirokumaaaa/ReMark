...
pick_jsonl() {
  local wm="$1"
  local c1="/mnt/personal_workspace/chenkeyu/ReMark/Attack-Face-Adapter/outputs_replay_large/generation_results_wm_aligned_${wm}_large.jsonl"
  local c2="/mnt/personal_workspace/chenkeyu/ReMark/Attack-Face-Adapter/outputs/generation_results_wm_aligned_${wm}.jsonl"
  local c3="/mnt/personal_workspace/chenkeyu/ReMark/Attack-Face-Adapter/outputs_replay_large/generation_results.jsonl"
...
PY
  ...
  if [[ -n "${FACE_JSONL_OVERRIDE:-}" ]]; then
    JSONL="${FACE_JSONL_OVERRIDE}"
  else
    JSONL="$(pick_jsonl "${WM}")"
  fi
  if [[ -z "${JSONL}" ]]; then
    echo "[skip] no replay jsonl for wm=${WM} run=${NAME}"
    continue
  fi
...
PY
  ...
  DST_CSV="${RUN_DIR}/transfer_eval_face_adapter_replay.csv"
  cp "${SRC_CSV}" "${DST_CSV}"
...
echo "[$(date '+%F %T')] ALL DONE face_adapter replay generalization (5 WM runs)."
