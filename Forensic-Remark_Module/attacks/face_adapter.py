...
class FaceAdapterAttack(BaseAttack):
    """
    Face-Adapter deepfake replay adapter.

    Face-Adapter 推理通常是“目录输入 + 慢速批量生成”，不适合在 DataLoader worker 里在线跑。
    本 adapter 读取预先生成的换脸输出（JSONL + 图像文件），在训练时做离线 replay。
    ...
    Config options (under attack_options):
        face_adapter_results_jsonl : path to the JSONL file (default: Attack-Face-Adapter/outputs/generation_results.jsonl)
        face_adapter_outputs_base  : base dir for relative path resolution (default: Attack-Face-Adapter)
        face_adapter_allow_missing : if True, fall back to wm_image when key not found (default: False)
        face_adapter_replay_key    : JSONL field used as lookup key (default: 'source_image')
    ...
    def _build_replay_map(self, jsonl_path: str) -> dict:
        if not os.path.exists(jsonl_path):
            raise FileNotFoundError(
                f"[FaceAdapterAttack] results jsonl not found: {jsonl_path}\n"
                f"Please generate Face-Adapter outputs first, then write a JSONL mapping and set "
                f"attack_options.face_adapter_results_jsonl accordingly."
            )
        replay = {}
    ...
    def generate(self, preprocessed: torch.Tensor) -> torch.Tensor:
        # Path-aware replay uses attack_with_cover(..., batch=...).
        return preprocessed
    ...
    def attack_with_cover(self, wm_images: torch.Tensor, cover_images: torch.Tensor, batch=None) -> torch.Tensor:
        if batch is None or 'img_path' not in batch:
            if self.allow_missing:
                return wm_images
            raise KeyError(
                "[FaceAdapterAttack] batch['img_path'] is required for replay lookup."
            )
        ...
        for i in range(b):
            ...
            if fake_path is None:
                if self.allow_missing:
                    ...
                    continue
                raise KeyError(
                    f"[FaceAdapterAttack] missing replay for target: {key}\n"
