...
_DEFAULT_RESULTS_JSONL = os.path.join(
    _PROJECT_ROOT, 'Attack-arc2face_wrapper', 'outputs', 'generation_results.jsonl'
)
...
class Arc2FaceAttack(BaseAttack):
    """
    Arc2Face deepfake replay adapter.

    It replays images generated by Attack-arc2face_wrapper.
    Default lookup key is `source_image` so each current sample maps to an output
    that should preserve its identity/background while transferring expression.
    """
    ...
    def __init__(self, cfg):
        ...
        self.replay_key = str(getattr(opts, 'arc2face_replay_key', 'source_image'))
