...
optim = torch.optim.Adam(params_trainable, lr=c.warmup_lr, betas=c.betas, eps=1e-6, weight_decay=c.weight_decay)
...
for i_epoch in range(c.epochs):
    if i_epoch < c.warmup_epochs:
        stage_name = "warmup"
        stage_lr = c.warmup_lr
        stage_message_weight = c.warmup_message_weight
        stage_stego_weight = c.warmup_stego_weight
    else:
        stage_name = "main"
        stage_lr = c.lr
        stage_message_weight = c.message_weight
        stage_stego_weight = c.stego_weight
    ...
    for idx_batch, cover_img in enumerate(trainloader):
        ...
        guass_noise = torch.zeros(left_noise.shape, device=device)
        ...
        total_loss = stage_message_weight * message_loss + stage_stego_weight * stego_loss
        total_loss.backward()
    ...
    logger_train.info(
        ...
        f"message_weight: {stage_message_weight} | "
        f"stego_weight: {stage_stego_weight}"
    )
    logger_train.info(
    ...
    with torch.no_grad():
        ...
        for test_cover_img in testloader:
            ...
            test_z_guass_noise = torch.zeros(test_left_noise.shape, device=device)
