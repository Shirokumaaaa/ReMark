...
To train with the prompt-only `Stable-Diffusion-Prompts` dataset included in this repo, use:

```python
!python train.py --message_len 48 --config configs/SD14_LaWa_prompt_dataset.yaml --batch_size 1 --max_epochs 40 --learning_rate 0.00006
```

This config renders prompt images with Stable Diffusion on the fly and caches them under `Stable-Diffusion-Prompts/cache`. Make sure the Stable Diffusion v1.4 checkpoint is available at `weights/stable-diffusion-v1/model.ckpt` before training.
