...
from tools.dataset import detect_prompt_column, read_parquet_with_hint
...
def load_prompt_batches(source_path, batch_size, prompt_column=None, prompt_limit=None):
    if source_path.lower().endswith(".parquet"):
        prompt_df = read_parquet_with_hint(source_path)
        ...
        print(f"Loaded {len(prompts)} prompts from parquet column '{prompt_column}'")
    else:
        with open(source_path, "r") as f:
            prompts = [prompt.strip() for prompt in f.read().splitlines() if prompt.strip()]
        print(f"Loaded {len(prompts)} prompts from text file")

    if prompt_limit is not None:
        prompts = prompts[:prompt_limit]
        print(f"Using the first {len(prompts)} prompts from {source_path}")

    if len(prompts) == 0:
        raise ValueError(f"No valid prompts were found in {source_path}")

    return list(chunk(prompts, batch_size))
...
def main(args):
    ...
    results.to_csv(os.path.join(outpath, "test_results_quality.csv"), index=False)
    attack_results.to_csv(os.path.join(outpath, "test_results_attacks.csv"), index=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Prompt column name when --from_file points to a parquet dataset",
    )
    parser.add_argument(
        "--prompt_limit",
        type=int,
        default=None,
        help="Optional maximum number of prompts to read from --from_file",
    )
