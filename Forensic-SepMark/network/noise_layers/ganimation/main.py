            ...
            with open(animation_attributes_path, 'r') as txt_file:
                ...
                for line in csv_lines:
                    ...
                    if not os.path.exists(image_path):
                        continue
                    _ = regular_image_transform(Image.open(image_path)).cuda()
                    reference_expression_images.append(splitted_lines[0])
                    targets.append(torch.Tensor(np.array(list(map(lambda x: float(x) / 5., splitted_lines[1::])))))

            if len(targets) == 0:
                raise RuntimeError("No valid GANimation attribute images found.")
