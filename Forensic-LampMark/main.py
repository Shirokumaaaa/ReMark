...
def train_common():
    ...
    for epoch in range(configs.epochs):
        ...
        if configs.do_validation:
            ...
            save_count = min(len(val_loader), configs.save_img_nums)
            save_iters = np.random.choice(np.arange(len(val_loader)), size=save_count, replace=False)
            save_imgs = None
            batch_count = 0
...
def tune_deepfake():
    ...
    if getattr(configs, 'pretrain_discriminator_path', None):
        discriminator_path = configs.pretrain_discriminator_path
    else:
...
if __name__ == '__main__':
    run_configs = load_run_config()

    # Pre-train using benign manipulations.
    if run_configs.do_pretrain:
        train_common()
    print("Train has completed or skipped")
    # Fine-tune with Deepfake manipulations.
    if run_configs.do_tune_deepfake:
        tune_deepfake()
    print("Fine-tune has completed or skipped")

    # Test benign.
    if getattr(run_configs, 'do_test_common', True):
        test_all_common()

    # Test Deepfake.
    if getattr(run_configs, 'do_test_simswap', True):
        test_simswap()
    # test_infoswap()
    # test_uniface()
    # TODO: gather and write code for E4S
    if getattr(run_configs, 'do_test_stylemask', True):
        test_stylemask()
    # test_stargan()
