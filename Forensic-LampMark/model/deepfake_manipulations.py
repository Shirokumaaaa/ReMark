...
# Ensure local model subrepos are importable regardless of CWD.
_BASE_DIR = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, os.path.join(_BASE_DIR, 'SimSwap'))
sys.path.insert(0, os.path.join(_BASE_DIR, 'stargan'))
sys.path.insert(0, os.path.join(_BASE_DIR, 'stylemask'))

# SimSwap utilities.
from .SimSwap.models.models import create_model
from .SimSwap.options.test_options import TestOptions

# InfoSwap utilities (optional).
try:
    ...
    Backbone128 = IIB = AII512 = UnetDecoder512 = MTCNN = None

# UniFace utilities (optional).
try:
    from UniFace.generate_swap import Model as UniFace
except Exception:  # pragma: no cover - optional dependency
    UniFace = None

# StyleMask utilities are imported lazily in StyleMaskModel to avoid
# failing module import when optional dependencies are missing.
...
class SimSwapModel(nn.Module):
    ...
    def __init__(self, img_size=128, mode='test'):
        ...
        try:
            ...
            cuda_ok = False
        if not cuda_ok:
            raise RuntimeError("SimSwap requires CUDA, but CUDA is not available in this environment.")
        opt = TestOptions().parse()
        # Force SimSwap paths to this repo's layout
        simswap_root = os.path.join(_BASE_DIR, 'SimSwap')
        opt.Arc_path = os.path.join(simswap_root, 'arcface_model', 'arcface_checkpoint.tar')
        opt.checkpoints_dir = os.path.join(simswap_root, 'checkpoints')
...
class StyleMaskModel(nn.Module):
    def __init__(self, img_size, device, mode='test'):
        ...
        try:
            ...
            raise RuntimeError(
                "StyleMask dependencies failed to import. Ensure StyleMask requirements and "
                "torch C++/CUDA extensions can build in this environment."
            ) from e
        # expose for forward()
        self._invert_image = invert_image
        self._generate_image = generate_image
        self._generate_new_stylespace = generate_new_stylespace
        self._decoder = decoder

        seed = 0
        random.seed(seed)

        stylemask_root = os.path.join(_BASE_DIR, 'stylemask')
        self.masknet_path = os.path.join(stylemask_root, 'pretrained_models', 'mask_network_1024.pt')
        ...
        # Resolve generator path to local pretrained_models.
        self.generator_path = stylegan2_ffhq_1024['gan_weights']
        if not os.path.isabs(self.generator_path):
            self.generator_path = os.path.join(stylemask_root, self.generator_path)
        if not os.path.exists(self.generator_path):
            alt_candidates = [
                os.path.join(stylemask_root, 'pretrained_models', 'stylegan2_ffhq_1024.pt'),
                os.path.join(stylemask_root, 'pretrained_models', 'stylegan2-ffhq-config-f_1024.pt'),
            ]
            for p in alt_candidates:
                if os.path.exists(p):
                    self.generator_path = p
                    break
        ...
        self.fa = face_alignment.FaceAlignment(face_alignment.LandmarksType.TWO_D, device=fa_device)
        ...
        if not os.path.isabs(self.encoder_path):
            self.encoder_path = os.path.join(stylemask_root, self.encoder_path)
        if not os.path.exists(self.encoder_path):
            alt_e4e = os.path.join(stylemask_root, 'pretrained_models', 'e4e_ffhq_1024.pt')
            if os.path.exists(alt_e4e):
                self.encoder_path = alt_e4e
    ...
    def reenact_pair(self, source_code, target_code):
        with torch.no_grad():
            ...
            source_img, style_source, w_source, noise_source = self._generate_image(self.G, source_code, self.truncation,
            ...
            target_img, style_target, w_target, noise_target = self._generate_image(self.G, target_code, self.truncation,
                                                                              self.trunc, self.image_resolution,
                                                                              self.split_sections,
                                                                              input_is_latent=self.input_is_latent,
                                                                              return_latents=True,
            ...
            new_style_space = self._generate_new_stylespace(style_source, style_target, mask,
            ...
            reenacted_img = self._decoder(self.G, new_style_space, w_source, noise_source, resize_image=self.resize_image)
    ...
    def forward(self, img_wm_device):
        ...
        inv_image, source_code = self._invert_image(img_source, self.encoder, self.G, self.truncation, self.trunc)
        inv_image, target_code = self._invert_image(img_wm, self.encoder, self.G, self.truncation, self.trunc)
        source_img, target_img, reenacted_img_wm = self.reenact_pair(target_code, source_code)

        if self.mode != 'train':
            return reenacted_img_wm
        else:
            inv_image, target_code = self._invert_image(img, self.encoder, self.G, self.truncation, self.trunc)
...
class StarGanModel(nn.Module):
    def __init__(self, img_size, mode='test'):
        ...
        # Solver overrides nn.Module.train with a custom signature. That breaks eval() on this wrapper.
        # Restore the base nn.Module train/eval methods on the instance for inference usage.
        self.solver.train = nn.Module.train.__get__(self.solver, nn.Module)
        self.solver.eval = nn.Module.eval.__get__(self.solver, nn.Module)
        # Load pretrained weights if available.
        try:
            import os
            ckpt_path = os.path.join(self.args.checkpoint_dir, f"{self.args.resume_iter:06d}_nets_ema.ckpt")
            if os.path.exists(ckpt_path):
                ckpt = torch.load(ckpt_path, map_location="cpu")
                # Load both nets and nets_ema with strict=False to tolerate optional keys (e.g., hpf.filter).
                for name, net in self.solver.nets.items():
                    if name in ckpt:
                        net.load_state_dict(ckpt[name], strict=False)
                for name, net in self.solver.nets_ema.items():
                    if name in ckpt:
                        net.load_state_dict(ckpt[name], strict=False)
        except Exception as e:
            print("StarGAN checkpoint load skipped:", e)
        # Use EMA networks for inference if available.
        self.generator = getattr(self.solver, "generator_ema", self.solver.generator)
        self.style_encoder = getattr(self.solver, "style_encoder_ema", self.solver.style_encoder)
        self.fan = getattr(self.solver, "fan_ema", self.solver.fan)
        self.star_gan = self.solver
    ...
    def reenactment(self, source, target):
        masks = self.fan.get_heatmap(source) if self.args.w_hpf > 0 else None
        # StarGAN v2 style encoder expects a domain label (y_trg), not an image.
        y_trg = torch.randint(2, size=(source.shape[0],), dtype=torch.long, device=source.device)
        s_ref = self.style_encoder(target, y_trg)
        x_fake = self.generator(source, s_ref, masks=masks)
        return x_fake
    ...
    def forward(self, img_wm_device):
        ...
        reenact_img_wm = self.reenactment(img_source, img_wm)
        if self.mode == 'train':
            reenact_img = self.reenactment(img_source, img)
