...
class ImageDataset(Dataset):
    def __init__(self, path_img, path_wm, img_size, wm_len, mode='train', manifest_path=None, base_dir=None):
        ...
        if manifest_path:
            ...
            manifest_abs = manifest_path if os.path.isabs(manifest_path) else os.path.join(base, manifest_path)
            self.manifest_items = []
            with open(manifest_abs, 'r') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    img_p = row.get('img_path') or row.get('img') or row.get('image')
                    wm_p = row.get('wm_path') or row.get('wm') or row.get('watermark')
                    if not img_p or not wm_p:
                        continue
                    if not os.path.isabs(img_p):
                        img_p = os.path.join(base, img_p)
                    if not os.path.isabs(wm_p):
                        wm_p = os.path.join(base, wm_p)
                    self.manifest_items.append((img_p, wm_p))
        else:
    ...
    def __getitem__(self, idx):
        if self.manifest_items is not None:
            while True:
                img_path, wm_path = self.manifest_items[idx]
                img = Image.open(img_path).convert('RGB')
                wm = np.load(wm_path)
                if img is not None and wm is not None:
                    img = self.transform_image(img)
                    return img, torch.Tensor(wm)
                print('Somehow skipped the image:', img_path, wm_path)
                idx += 1
        else:
            while True:
...
def make_loader(configs, model_mode='train', shuffle=True):
    ...
    """
    manifest_path = getattr(configs, 'manifest_path', None)
    if manifest_path:
        manifest_path = manifest_path.format(split=model_mode)
    ...
    if len(dataset) == 0:
        raise ValueError(
            f"Empty dataset for split={model_mode}. "
            f"Check manifest_path={manifest_path}, img_path={configs.img_path}, wm_path={configs.wm_path}."
        )
    drop_last = (model_mode == 'train') and (len(dataset) >= configs.batch_size)
    loader = DataLoader(dataset, batch_size=configs.batch_size, num_workers=12, shuffle=shuffle, drop_last=drop_last)
    return loader
