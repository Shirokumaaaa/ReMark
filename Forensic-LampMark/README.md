...
### CSV-based matching (recommended)
If your `image_data` and `watermark_data` are not perfectly aligned by filename, you can use a CSV manifest to define matched pairs without moving any files.

We provide manifests under `data_manifests/`:
```
data_manifests/
  celeba_hq_128_{train|val|test}.csv
  celeba_hq_256_{train|val|test}.csv
```

Each CSV contains:
```
img_path,wm_path
image_data/dataset_celeba_hq/train/000123.jpg,watermark_data/celeba_hq/256/train/000123.npy
```

To enable it, add this line in your config JSON (e.g. `pretrain.json` / `tune_deepfake.json` / `test_*.json`):
```
"manifest_path": "./data_manifests/celeba_hq_256_{split}.csv"
```
Use `celeba_hq_128_{split}.csv` if you want 128-size watermarks instead.
            ├── AJ_Cook_0001.jpg
            └──...
```
