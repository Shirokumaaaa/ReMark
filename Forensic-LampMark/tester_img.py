...
class TesterImg:
    ...
    def _deepfake_manipulation_multi_gpu(self):
        if self.sim_swap is not None:
            self.sim_swap = nn.DataParallel(self.sim_swap)
        if self.info_swap is not None:
            self.info_swap = nn.DataParallel(self.info_swap)
        if self.uni_face is not None:
            self.uni_face = nn.DataParallel(self.uni_face)
        if self.e4s is not None:
            self.e4s = nn.DataParallel(self.e4s)
        if self.star_gan is not None:
            self.star_gan = nn.DataParallel(self.star_gan)
        # StyleMask has internal tensors tied to a single device; avoid DataParallel to prevent device mismatch.
        if self.hyper_reenact is not None:
    ...
    def test_batch_stylemask(self, imgs, wms):
        ...
        with torch.no_grad():
            ...
            if reenacted_img_wm.shape[-1] != self.img_size:
                reenacted_img_wm = torch.nn.functional.interpolate(
                    reenacted_img_wm, size=(self.img_size, self.img_size), mode='bilinear', align_corners=False
                )
